...
int main()
{
    ...
    int expectedValues[numTestValues]{ -1, 0, 3, -1, -1, 8, -1, 13, -1 }; // if not found, assign -1

    // Loop through all of the test values
    for (int count{ 0 }; count < numTestValues; ++count)
    {

        int index_of_bs=binarySearch(array, testValues[count], 0, static_cast<int>(std::size(array)) - 1);
        // See if our test value is in the array
        int index{ index_of_bs };
...
// Related page: https://www.learncpp.com/cpp-tutorial/chapter-20-summary-and-quiz/
